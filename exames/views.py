...
def solicitar_exames(request):
    ...
    if request.method == "GET":
        ...
        # Calcula preço de todos os exames selecionados
        preco_total_selecionados = 0
        for i in solicitacao_exames:
            if i.disponivel:
                preco_total_selecionados += i.preco

        return render(request, 'solicitar_exames.html', {
            'solicitacao_exames': solicitacao_exames,
            'preco_total_selecionados': preco_total_selecionados,
...
# TODO: validar se o exame é do usuário [feito]
    if not exame.requer_senha:
        ...
        if not exame.resultado:
            messages.add.message(request, constants.ERROR, 'Ainda não foi cadastrado seu resultado, entre em contato com o laboratório.')
